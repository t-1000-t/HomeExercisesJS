/*
Используя массив объектов пользователей
users (дальше в задании), напиши функции которые с помощью перебирающих методов
массивов (никаких for, splice и т. д.) выполняют следующие операции.
...
const getAllNames = users => users.map(elem => elem.name);
...
const getUsersByEyeColor = (users, color) => users.filter((user) => {
...
const getUserByEmail = (users, email) => users.find((user) => {
  if (user.email === email) {
    return {
      ...user,
    };
...
const getUsersByFriend = (users, name) => {
  const arrUser = [];
  users.map((user) => {
    if (Object.values(user.friends).includes(name)) {
      arrUser.push(user.name);
    }
  });
  return arrUser;
